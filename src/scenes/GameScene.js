...
export class GameScene extends Phaser.Scene {
  ...
  _buildRoom() {
    ...
    // Walls (visual)
    const walls = this.add.graphics().setDepth(DEPTHS.FLOOR)
    walls.fillStyle(0x1a2e28)
    walls.fillRect(0, 0, W, T * 2)           // top
    walls.fillRect(0, H - T * 2, W, T * 2)   // bottom
    walls.fillRect(0, 0, T * 2, H)            // left
    walls.fillRect(W - T * 2, 0, T * 2, H)   // right

    // Stone border lines
    walls.lineStyle(2, 0x374151, 1)
    walls.strokeRect(T * 2, T * 2, W - T * 4, H - T * 4)
  ...
  _spawnEntities() {
    ...
    ;[{ x: 700, y: 180 }, { x: 740, y: 380 }, { x: 200, y: 200 }].forEach(pos => {
    ...
    this.physics.add.overlap(
      ...
      (proj, enemy) => { enemy.takeDamage(proj.damage); proj._gfx?.destroy(); proj.destroy() }
    )
  }
